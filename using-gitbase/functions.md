...
| `REPLACE(str,from_str,to_str)` | Returns the string str with all occurrences of the string from\_str replaced by the string to\_str. |
...
## Note about uast, uast\_mode, uast\_xpath and uast\_children functions
...
> uast\_extract\(nodes\_column, @common\_selector\)
...
> uast\_extract\(nodes\_column, 'some-property'\)
...
The result returned by this function is a JSON, which means to access its fields, the use of \`JSON\_EXTRACT is needed.
