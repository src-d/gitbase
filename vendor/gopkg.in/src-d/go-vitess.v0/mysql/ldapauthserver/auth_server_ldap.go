...
import (
	...
	"gopkg.in/src-d/go-vitess.v0/netutil"
...
func Init() {
	...
	if *ldapAuthMethod != mysql.MysqlClearPassword && *ldapAuthMethod != mysql.MysqlDialog {
		log.Panicf("Invalid mysql_ldap_auth_method value: only support mysql_clear_password or dialog")
	}
	...
	if *ldapAuthConfigFile != "" {
		...
		if err != nil {
			log.Panicf("Failed to read mysql_ldap_auth_config_file: %v", err)
		}
	}
	if err := json.Unmarshal(data, ldapAuthServer); err != nil {
		log.Panicf("Error parsing AuthServerLdap config: %v", err)
	}
